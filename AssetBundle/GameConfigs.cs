...
public static class GameConfigs
{
	...
	//(该文件夹只能读，打宝石被一起写入包内，第一次运行游戏把该文件夹数据拷贝到本地ab包路径下)
	...
	//打包资源的输出文件夹(导出到streamingasset文件夹下)
	public static string GameResExporPath = Application.streamingAssetsPath + "/" + curPlatformName;
	...
	//models预制体路径
	public static string GetModelAtlasPath(string name)
	{
		string str = "/Prefabs/" + name;
		if (LoadAssetMode != FoxGame.Asset.AssetLoadMode.Editor)
		{
			str = str.ToLower();
		}
